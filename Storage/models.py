...
class Delivery(Base):
  ...
  requested_date = mapped_column(DateTime, nullable=False, default=lambda: datetime.now())
...
class Schedule(Base):
  ...
  created_date = mapped_column(DateTime, nullable=False, default=lambda: datetime.now())
  trace_id = mapped_column(String(250), nullable=False)
